...
export default function HistoryBaejjangPage() {
  const member = {
    name: "배짱",
    realName: "박종배",
    team: "1조",
  ...
  const character = {
    ...
    description:
      "공유회도 5번이나 다니고, SNS 인증도 꾸준히 했지만, 배짱이 진짜 많이 한 건 셸 보내기. 25번이나 다른 크루에게 셸을 보냈다. 본인이 \"AI를 쉽게 진정성있게 풀어내는 사람\"으로 기억되고 싶다고 했는데, 실제로 말보다 응원으로 먼저 움직이는 사람이었다.",
  };
  ...
  const beforeAfter = {
    ...
    after: "6주간 한 번도 빠지지 않고 출석했다. FFmpeg.wasm이 4번 실패했을 때 git tag로 안전하게 되돌리고, 30분 후 Vercel Function으로 해결했다. 에러 앞에서 멈추는 사람이 아니라, 에러를 기록하고 다음 라운드를 만드는 사람이 되어 있었다.",
  };
  ...
  const timeline = [
    ...
    },
    {
      ...
      summary: "os-interview 스킬로 60분간 인터뷰를 받고, OS 선언문과 첫 부품(campaign-result-engine)을 만들었다. \"가설을 진화시키고 새 영역을 여는 일만 하고, 검증·추적·다음 후보 제안은 시스템이 한다.\" 에러 7개를 만나 7개를 해결하면서, AI를 도구가 아니라 사고 동반자로 쓰는 전환점을 발견했다.",
    ...
    },
    {
      ...
      summary: "Karpathy의 LLM Wiki 패턴을 벤치마킹해 4-layer 지식 시스템을 만들었다. 그런데 진짜 발견은 구조가 아니라 원리였다. v0를 덧칠해서 v1으로 라벨만 바꾸려다 멈췄다 — \"가설이 같은 자리에 묵묵히 덧칠되는 것\"이 본인의 통점이었는데, 시스템 빌드 자체가 그 통점을 반복하고 있었다. 이 발견이 \"메타 정합성\" 규범으로 박혔다.",
    ...
    },
    {
      ...
      summary: "정체성을 전환했다. 본부장(회사)이 아니라 아들로서, 엄마에게 매일 따뜻한 한 줄을 보내는 손글씨 카드 도구를 빌드했다. Claude Code 스킬(CLI)과 GitHub Pages 웹페이지 두 인터페이스로 출시. 하네스와 오케스트레이션의 정의를 사전에 박지 않고, 빌드하면서 손에 잡았다.",
    ...
    },
    {
      ...
      summary: "워크샵에서 흡수한 \"디자인 MD + 콘텐츠 MD 두 축\" 원리를 적용해 디자인 시스템을 SSoT로 박았다. 와이프(\"좋으네유\")와 교회동료(3층 분석 피드백)에게 카드를 보냈더니 같은 도구가 완전히 다른 결로 받아졌다. 교회동료의 피드백 두 가지를 마감 1시간 전 즉시 반영 — 성경 구절 추천과 관계 메타데이터.",
    ...
    },
    {
      ...
      summary: "오프라인 모임에서 받은 7개 피드백을 5가지 즉시 + 2가지 다음 라운드로 분류했다. 보이스 녹음, mp4 변환(FFmpeg.wasm 4번 실패 후 Vercel Function으로 해결), 텔레그램 자동 발송, 펜팔 사서함까지 확장. 그리고 마감 30분 전, 첫 영상편지를 엄마에게 보냈다. 답이 왔다 — \"아들 고마워요 고추소독하셨어요,,\"",
    ...
    <div className="min-h-screen bg-[var(--paper)]">
      ...
      <section className="bg-[var(--yellow)]">
        <div className="max-w-3xl mx-auto px-6 py-12 md:py-16 text-center">
          ...
          <p className="text-sm text-[var(--ink-80)] leading-relaxed max-w-lg mx-auto">
