...
export default function RiboCertificate() {
  ...
  const giftMessages = [
    { sender: "달빛그린", message: "열심히 과제하는 모습 멋져요~" },
  ...
  const timeline = [
    ...
    {
      week: "4주차", date: "5/31",
      title: "스크랩어데이 — Post-MVP 폴리시 + 이메일 로그인",
      summary: "특별한 날 17개 키치 SVG 스티커, per-stamp 토글, 배경 컬러 자유 선택, 검색 통계+월별 collapse를 추가했어요. Supabase 매직링크로 이메일 로그인을 구현하고, 디자인 규칙(폰트 크기, 스타일 가이드)을 명확히 정해서 일관성을 잡았어요.",
      insight: "사용자가 본인 워크플로우를 가장 잘 안다. \"왜 이 위치에 이 기능?\"이라고 물어보지 않았으면 그대로 ship됐을 것.",
    },
    {
      week: "5주차", date: "6/7",
      title: "스크랩어데이 계속 다듬기 + 이벤트페이지 구글시트 연동",
      summary: "스크랩어데이 스티커·토글·통계 기능을 계속 다듬고, 별도 이벤트페이지의 이름 변환·기프트 뽑기·설문 데이터를 구글시트에 연동하고 분석 대시보드까지 구현했어요. 카페에서 만난 중국인에게 언어 검수를 받아 샤오홍슈 계정도 개설.",
      insight: "고객 인터뷰가 중요하다. 중국인들은 인스타가 아니라 샤오홍슈를 쓴다는 걸 우연히 알게 됐다.",
    },
  ];
      ...
      {hasDiploma && (
        <section className="bg-[var(--ink-05)] border-b border-[var(--ink-10)]">
          <div className="max-w-2xl mx-auto px-6 py-12 md:py-16">
            <div className="bg-[#FFFBEB] border-2 border-[#F59E0B] p-8 md:p-12 text-center relative"><RankBadge memberName="리보(이보경)" />
