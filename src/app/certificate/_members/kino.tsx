...
export default function KinoCertificate() {
  ...
  const character = {
    type: "한번에 하나씩 디테일하게 파는 크루",
    image: "/certificate/character-star.svg",
    description:
      "0, 1, 3주차 과제를 제출하며(부조장) 스폰지타임즈 카드뉴스 콘텐츠 제작 OS(롱폼+숏폼)를 만들고, 3주차에 마인크래프트 스타일 나노바나나 캐릭터로 카드뉴스 8개를 제작했어요. \"여러가지보다 한번에 하나씩 디테일하게\"라는 집중의 철학을 보여준 사람이에요. 셸 40개를 받으며 전체 멤버 중 2위, 공유회 9번 참여.",
  };
  ...
  const badges = [
    ...
    { name: "셸 인기스타", icon: "/badges/shell-superstar.svg", earned: "2026.06" },
  ];

  const giftMessages = [
    { sender: "코니", message: "리모션 스킬로 다양한 결과물 만들어서 공유해주신 거 재미있게 봤어요!" },
    { sender: "박상임", message: "응급실 열정 미쳤당... 얼른 나으세요!" },
  ...
  const timeline = [
    {
      week: "1주차", date: "5/10",
      title: "스폰지타임즈 카드뉴스 콘텐츠 제작 OS",
      summary: "롱폼과 숏폼을 아우르는 카드뉴스 콘텐츠 제작 OS를 설계했어요. 스폰지타임즈 프로젝트의 콘텐츠 파이프라인을 잡았어요.",
    },
    {
      week: "3주차", date: "5/24",
      title: "카드뉴스 8개 제작 — 마인크래프트 스타일 나노바나나 캐릭터",
      summary: "마인크래프트 스타일의 나노바나나 캐릭터를 활용해 카드뉴스 8개를 제작했어요. 한번에 하나씩 디테일하게 파는 접근으로 품질을 높였어요.",
      insight: "여러가지보다 한번에 하나씩 디테일하게.",
    },
    {
      week: "4주차", date: "5/31",
      title: "easyCard — 하네스 + 브랜드 템플릿 시스템 구축",
      summary: "easyCard에 CLAUDE.md·PRD.md 하네스를 추가하고 브랜드 레지스트리(registry.js) 시스템을 도입했어요. 코드 수정 없이 브랜드 추가 가능한 구조로 전환. Vercel 자동배포가 6일째 멈춰 있어서 CLI 직접 배포로 해결했어요.",
      insight: "하네스는 배포되는 웹과 별개다. 내 AI 직원이 매번 같은 기준으로 일하게 하는 받침대.",
    },
    {
      week: "6주차", date: "6/14",
      title: "카드뉴스 툴 갤러리 등록 + 미니유닛 운영",
      summary: "카드뉴스 포맷 생성 툴을 갤러리에 등록했어요. 블로그 유닛(SEO 규칙 기반 매일 포스팅)과 나만의 OS 만들기 유닛(오픈클로·헤르메스 로컬 AI + 슬랙 연동)을 운영했어요.",
      insight: "로컬 AI는 대화 기반으로 다음 행동을 스스로 판단하는 구조라 개인 에이전트 만들기에 적합하다.",
    },
  ];

  const beforeAfter = {
    before: `원하는 결과를 얻는\n프롬프트, 이ㅡ결과물이 유저입장에서 어떻게\n보이는 지, 쓸만한지 궁금함\n사람들에게 필요한 서비스를 어떻게 파악하고 만들지`,
    after: "스폰지타임즈 카드뉴스 콘텐츠 제작 OS를 만들고 마인크래프트 스타일 카드뉴스 8개를 제작했어요. \"여러가지보다 한번에 하나씩 디테일하게\"라는 집중의 방법론을 찾은 사람이에요.",
  };
      ...
      {hasDiploma && (
        <section className="bg-[var(--ink-05)] border-b border-[var(--ink-10)]">
          <div className="max-w-2xl mx-auto px-6 py-12 md:py-16">
            <div className="bg-[#FFFBEB] border-2 border-[#F59E0B] p-8 md:p-12 text-center relative"><RankBadge memberName="키노(강은주)" />
