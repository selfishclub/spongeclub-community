...
export default function LindiCertificate() {
  ...
  const character = {
    ...
    description:
      "0-6주차 과제를 전부 제출했어요. 상세 내용은 제한적이지만 매주 빠짐없이 꾸준히 과제를 제출하며 자리를 지킨 사람이에요.",
  };
  ...
  const timeline = [
    {
      week: "1주차", date: "5/10",
      title: "0-6주차 과제 제출 시작",
      summary: "0-6주차 과제를 꾸준히 제출했어요. 상세 내용은 제한적이지만 매주 빠짐없이 참여하며 끝까지 함께한 사람이에요.",
    },
  ];
  ...
  const giftMessages = [
    ...
    { sender: "그린", message: "린디님이 클라우드 방식 알려주셔서 어떤 개념인지 알 수 있었어요-!" },
  ];

  const beforeAfter = {
    before: "리서치 후 기획단계를 거쳐 프로프트 작성을 했지만 원하는 만족스럽지 않은 이미지 결과물이 나오고 영상을 제작하는 수많은 툴중에 내가 편하게 선택한 툴이 아직 없어서인지 익숙지 않아 그또한 결과물이 맘에 안들고 어렵게 느껴집니다. 많은 경험을 해야 하는데 아직 그러지 못해서 라고 생각해요",
    after: "0-6주차 과제를 전부 제출하며 꾸준히 자리를 지켰어요. 끝까지 남은 사람이에요.",
  };
      ...
      {hasDiploma && (
        <section className="bg-[var(--ink-05)] border-b border-[var(--ink-10)]">
          <div className="max-w-2xl mx-auto px-6 py-12 md:py-16">
            <div className="bg-[#FFFBEB] border-2 border-[#F59E0B] p-8 md:p-12 text-center relative"><RankBadge memberName="린디(김효식)" />
