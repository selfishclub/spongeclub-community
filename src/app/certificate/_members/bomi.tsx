...
export default function BomiCertificate() {
  ...
  const character = {
    ...
    description:
      "0, 1주차 과제를 제출하며 cycle-os -- 여성 호르몬 주기 기반 자기관리 대시보드를 만들었어요. 5개 카테고리(화장품, 일정, 운동, 음식, 감정) 추천 시스템을 설계한 사람이에요. \"벤치마크는 도메인이 아니라 패턴을 빌리는 것\"이라는 통찰과 \"원래 계획에 매달리지 말 것\"이라는 교훈을 남겼어요.",
  };
  ...
  const giftMessages = [
    { sender: "박상임", message: "오늘의 셸을 보내드립니다! 한주간도 화이팅~~" },
    { sender: "박상임", message: "API까지 연결하시다니! 멋쟈요~~" },
    { sender: "헤이즐", message: "과제 추가로 공유하느라 바쁘셨겠어요 화이팅입니다!" },
  ];

  const timeline = [
    {
      week: "1주차", date: "5/10",
      title: "cycle-os — 여성 호르몬 주기 기반 자기관리 대시보드",
      summary: "여성 호르몬 주기에 따라 화장품, 일정, 운동, 음식, 감정 5개 카테고리를 추천하는 대시보드를 만들었어요. 회사 자료 접근 불가로 work-helper에서 피벗한 결과물이에요.",
      insight: "벤치마크는 도메인이 아니라 패턴을 빌리는 것. 원래 계획에 매달리지 말 것.",
    },
  ];

  const beforeAfter = {
    before: `- 원하는 퀄리티에 도달하는 과정과 방법\n- 브랜드의 여러 자산을 지식 베이스화해서 팀원들과 공유하는 방법 (매일 새롭게 발행되는 콘텐츠나 브랜드 관련 소식들을 지식 베이스에 자동으로 추가 및 아카이빙하는 방법)\n- 90% 이상 반복 업무를 자동화하고 정신적 소모를 줄일 수 있는 방법\n`,
    after: "cycle-os 호르몬 주기 기반 자기관리 대시보드를 만들며, 벤치마크는 도메인이 아니라 패턴을 빌리는 것이라는 관점을 얻었어요. 원래 계획에 매달리지 않고 피벗하며 답을 찾아간 사람이에요.",
  };
