...
export default function TurtleCertificate() {
  const member = {
    ...
    realName: "나병우",
    team: "5조",
    jobTitle: "1인 기업 / 이커머스 화장품&식품",
    period: "2026.05.03 — 2026.06.14",
  };

  const character = {
    type: "12명 시절보다 더 많은 일을 하는 사람",
    image: "/certificate/character-cheerleader.svg",
    description:
      "오프라인에서 \"LG에서 20년 근무 후 1인 사업을 시작했고, AI 덕분에 직원 12명 시절보다 더 많은 업무를 처리하고 있다\"고 밝혔다. 네이버 쇼핑 인사이트로 쌀 데이터를 분석해 누룽지 업체로부터 비즈니스 계획서를 제안받는 성과를 냈어요. 공유회 7번 참여, 셸 17개 보내기.",
  };
  ...
  const badges = [
    ...
    { name: "첫 공유회 오픈", icon: "/badges/first-session-host.svg", earned: "2026.05" },
  ];
  ...
  const beforeAfter = {
    ...
    after: "오프라인에서 \"AI로 쌀 데이터를 분석해 누룽지 업체 담당자들보다 쌀에 대해 더 깊이 파악하고 있다는 평가를 받았다\"고 밝혔다. 데이터를 DB화하는 고민이, 실제 비즈니스 제안으로 이어졌다.",
  };
