...
export default function HazelCertificate() {
  ...
  const character = {
    type: "가능 여부 먼저 확인하는 크루",
    image: "/certificate/character-cheerleader.svg",
    description:
      "0, 1, 2, 3주차 과제를 모두 제출하며 신사 맛집/카페 지도(shinsa-lunch-finder)를 Vercel에 배포하고, 텔레그램 봇 3종(#검증 태그, 날씨+주식 시황, 머스크 페르소나 회고)을 만들었어요. \"크롤링과 API 가능 여부를 먼저 확인한 후 기획\"하는 접근과 \"같은 토큰으로 봇 두 개 동시 실행 불가\"라는 실전 교훈을 남긴 사람이에요.",
  };
  ...
  const badges = [
    ...
    { name: "셸 5회 받기", icon: "/badges/shell-receive-5.svg", earned: "2026.06" },
    { name: "첫 SNS 인증", icon: "/badges/first-sns.svg", earned: "2026.05" },
  ];

  const giftMessages = [
    { sender: "오웬", message: "이번주 반차까지 각오하고 있는 헤이즐에게 응원을 보내요!!" },
  ...
  const timeline = [
    {
      week: "1주차", date: "5/10",
      title: "신사 맛집/카페 지도 — shinsa-lunch-finder Vercel 배포",
      summary: "신사동 맛집과 카페를 찾아주는 지도 서비스를 만들어 Vercel에 배포했어요. API 연동 서비스를 기획하면서 어떤 데이터를 활용할 수 있는지 우선 파악하는 과정을 거쳤어요.",
      insight: "크롤링과 API 가능 여부를 먼저 확인한 후 기획해야 해요.",
    },
    {
      week: "2주차", date: "5/17",
      title: "텔레그램 봇 #검증 태그 + 날씨/주식 시황 봇",
      summary: "#검증 태그로 정보를 필터링하는 봇과 날씨+주식 시황을 알려주는 텔레그램 봇을 만들었어요.",
      insight: "같은 토큰으로 봇 두 개를 동시 실행할 수 없다는 걸 배웠어요.",
    },
    {
      week: "3주차", date: "5/24",
      title: "머스크 페르소나 회고 봇",
      summary: "머스크 페르소나를 가진 텔레그램 회고 봇을 만들었어요. 텔레그램 봇 3종 시리즈의 마지막 작품이에요.",
    },
  ];

  const beforeAfter = {
    before: "어디서부터 시작할지 모르겠음",
    after: "신사 맛집 지도 Vercel 배포에서 시작해 텔레그램 봇 3종까지 만들었어요. \"크롤링과 API 가능 여부를 먼저 확인 후 기획\"하는 원칙을 세우며, 어디서부터 시작할지 몰랐던 사람이 실전 검증 후 만드는 사람이 되었어요.",
  };
