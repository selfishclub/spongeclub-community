...
export default function BaejjangCertificate() {
  ...
  const timeline = [
    ...
    },
    {
      ...
      summary: "Karpathy의 LLM Wiki 패턴을 벤치마킹해 4-layer 지식 시스템을 만들었어요. 그런데 진짜 발견은 구조가 아니라 원리였어요. v0를 덧칠해서 v1으로 라벨만 바꾸려다 멈췄다 — \"가설이 같은 자리에 묵묵히 덧칠되는 것\"이 본인의 통점이었는데, 시스템 빌드 자체가 그 통점을 반복하고 있었어요. 이 발견이 \"메타 정합성\" 규범으로 박혔다.",
    ...
    },
    {
      ...
      summary: "워크샵에서 흡수한 \"디자인 MD + 콘텐츠 MD 두 축\" 원리를 적용해 디자인 시스템을 SSoT로 박았어요. 와이프(\"좋으네유\")와 교회동료(3층 분석 피드백)에게 카드를 보냈더니 같은 도구가 완전히 다른 결로 받아졌다. 교회동료의 피드백 두 가지를 마감 1시간 전 즉시 반영 — 성경 구절 추천과 관계 메타데이터.",
